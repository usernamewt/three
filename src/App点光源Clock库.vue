<template>
  ...
  /* eslint-disable */
    ...
    pointLight.decay = 0;
    // 包含点光源的球
    const smallB = new Three.Mesh(
        new Three.SphereGeometry(0.1,20,20),
        new Three.MeshBasicMaterial({color:0xff0000})
    )
    const time = new Three.Clock();
    smallB.position.set(2,2,2);
    sence.add(smallB);
    smallB.add(pointLight); 
    // sence.add(ballo);
    // sence.add(ballt);
    // 设置相机的位置
    camera.position.z = 5;
    ...
    function animate(){
      controls.update();
      let line = time.getElapsedTime();
      smallB.position.x = Math.sin(line)
      smallB.position.z = Math.cos(line)
    ...
    gui.add(smallB.position,'x').max(10).min(-2).step(0.1)
